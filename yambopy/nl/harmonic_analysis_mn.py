...
# Build the M matrix
    ...
    for i_n in range(1, N+1):
        for i_m in range(0, floor(N/2)+1)
         j=j+1
...
def Harmonic_Analysis_nm(nldb, X_order=4, N_samp=-1, T_range=[-1, -1],prn_Peff=False,INV_MODE="full",prn_Xhi=True):
    ...
    if N%2=0: M_size =(X_order*(X_order+4)+2)/2
    else:
        M_size =(X_order*(X_order+4)+1)/2 
    L=(M_size-1)/2
    # Polarization response
    X_effective = np.zeros((L+1, n_runs, 3), dtype=np.cdouble)
    X_matrix = np.zeros((X_order + 1, X_order + 1, n_run, 3 ))
    ...
    # Current response
    if l_eval_current:
        Sigma_effective = np.zeros((L+1, n_runs, 3), dtype=np.cdouble)
        Sigma_matrix = np.zeros((X_order + 1, X_order + 1, n_run, 3 ))
    ...
    print (X_order, len(Harmonic_Frequency[:,1]))
    ...
    for i_f in tqdm(range(n_runs)):
        ...
        if  efield["name"]=="QSSIN":
            ...
            T_0=np.pi/Harmonic_Frequency[1, i_f]*float(round(Harmonic_Frequency[1, i_f]/np.pi*10.0*sigma))
            if T_range[0] <= 0.0:
                T_range[0] = T_0 - T_period/2
            if T_range[1] <= 0.0:
                T_range[1] = T_0 + T_period/2
        ...
        for i_d in range(3):
            # Fill matrices
            j = 0
            for n in range(X_order + 1): #order 
                for m in range(floor(n-1)/2 +1): # negative frequency
             ...
             # Calculate susceptibilities
                     if Efield_order[k, m, i_f, i_d] == 1:
                         Susceptibility[k, m , i_f, 0] = 4.0 * np.pi * np.dot(efield['versor'], X_matrix[k, m, i_f, :])
                         if l_eval_current:
                             Conductibility[k, m + 1,i_f,0]= 4.0*np.pi*np.dot(efield['versor'][:],Sigma_matrix[k, m ,i_f,:])
                     else:
    ...
    #-----------to be adapted to mn , L formalism---------------
   ...
   #             Conductibility[i_order, :, :] *= Junit/(EFunit**i_order) # i_order is the order of harmonics omega^{i_order}, not order of E-field.
    ...
    if prn_Xhi:
        print("Write final results: xhi^1, xhi^2, xhi^3, etc...")
        for k in range(X_order+1):
           for m in range(X_order+1):
               output_file = f'o{prefix}.YamboPy-X_probe_order_{k+m}{m}'
